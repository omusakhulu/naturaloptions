...
const invoiceStatusObj = {
  ...
  'Pending Payment': { color: 'warning', icon: 'tabler-clock' },
  Completed: { color: 'success', icon: 'tabler-badge-check' }
}
    ...
    () => [
      ...
      columnHelper.accessor('invoiceStatus', {
        ...
        cell: ({ row }) => (
          ...
          >
            {(() => {
              const key = normalizeStatusLabel(row.original.invoiceStatus)
              const meta = invoiceStatusObj[key] || invoiceStatusObj['Draft']

              return (
                <CustomAvatar skin='light' color={meta.color} size={28}>
                  <i className={classnames('bs-4 is-4', meta.icon)} />
                </CustomAvatar>
              )
            })()}
          </Tooltip>
