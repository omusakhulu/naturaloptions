...
// WooCommerce core statuses mapping -> MUI chip colors
...
const OrderListTable = ({ orderData }) => {
  ...
  const dynamicColorMap = useMemo(() => {
    const dyn = (statuses || []).reduce((acc, s, idx) => {
      const key = (s.key || '').toLowerCase()

      if (key && !statusChipColor[key]) {
        const palette = ['secondary', 'info', 'success', 'warning', 'primary', 'default']
  ...
  const columns = useMemo(
    () => [
        ...
        cell: ({ row }) => (
          <div className='flex items-center gap-3'>
            ...
            <div className='flex flex-col'>
              ...
              >
                {typeof row.original.customer === 'object' ? row.original.customer?.name : row.original.customer}
              </Typography>
      ...
      columnHelper.accessor('paymentMethodTitle', {
        header: 'Payment',
        cell: ({ row }) => <Typography className='font-medium'>{row.original.paymentMethodTitle || 'N/A'}</Typography>
      }),
      columnHelper.accessor('status', {
        header: 'Status',
        cell: ({ row }) => {
          const status = row.original.status
          const colorMap = dynamicColorMap
          const statusColor = colorMap[status]?.color || 'default'
              ...
              options={[
                {
                  ...
                  menuItemProps: {
                    onClick: () =>
                      router.push(getLocalizedUrl(`/apps/ecommerce/orders/details/${row.original.id}`, locale)),
  ...
  const filteredData = useMemo(() => {
    ...
    return (data || []).filter(o => String(o.status || '').toLowerCase() === key)
  }, [data, statusFilter])
