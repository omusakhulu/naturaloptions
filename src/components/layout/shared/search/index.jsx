  ...
  return (
    <>
      ...
      <CommandDialog open={open} onOpenChange={setOpen}>
        <div className='flex items-center gap-4 border-be pli-6 plb-4 bg-backgroundDefault relative'>
          <Title hidden />
          <Description hidden />
          <i className='tabler-search text-xl text-textSecondary' />
          <CommandInput 
            value={searchValue} 
            onValueChange={setSearchValue}
            placeholder='Search for pages, apps, settings...'
          />
          <kbd className='hidden sm:inline-flex text-textDisabled text-xs absolute left-1/2 -translate-x-1/2'>ESC</kbd>
          <i className='tabler-x cursor-pointer text-xl hover:text-error transition-colors' onClick={() => setOpen(false)} />
        </div>
        <CommandList>
          {searchValue ? (
