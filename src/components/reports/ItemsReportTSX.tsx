...
type ItemRow = { sku: string; name: string; quantity: number; amount: number }

type Data = { range: { from: string; to: string }; items: ItemRow[]; totals: { quantity: number; amount: number } }
...
export default function ItemsReportTSX({ lang = 'en' }: { lang?: string }) {
  const monthStart = useMemo(() => new Date(new Date().getFullYear(), new Date().getMonth(), 1), [])
  const today = useMemo(() => new Date(), [])
  const { params, setParams } = useReportQuery({ from: toDateInputValue(monthStart), to: toDateInputValue(today), q: '' })

  const [data, setData] = useState<Data>({ range: { from: toDateInputValue(monthStart), to: toDateInputValue(today) }, items: [], totals: { quantity: 0, amount: 0 } })
  const [loading, setLoading] = useState(false)
  const [error, setError] = useState('')
  const [sortBy, setSortBy] = useState<'name' | 'sku' | 'quantity' | 'amount'>('quantity')
  ...
  const fetchReport = async () => {
    ...
    try {
      ...
      // Mock
      const items: ItemRow[] = [
        { sku: 'SKU-001', name: 'Product A', quantity: 120, amount: 5400 },
        { sku: 'SKU-002', name: 'Product B', quantity: 60, amount: 1800 }
      ]
      const totals = items.reduce((s, r) => ({ quantity: s.quantity + r.quantity, amount: s.amount + r.amount }), { quantity: 0, amount: 0 })
      setData({ range: { from: params.from || toDateInputValue(monthStart), to: params.to || toDateInputValue(today) }, items, totals })
    } finally {
      setLoading(false)
    }
  }
  ...
  const setSort = (field: 'name' | 'sku' | 'quantity' | 'amount') => {
    if (sortBy === field) setSortDir(d => (d === 'asc' ? 'desc' : 'asc'))
    else { setSortBy(field); setSortDir('asc') }
    setPage(1)
  }

  const breadcrumbs = [ { label: 'Reports', href: `/${lang}/apps/reports` }, { label: 'Items Report' } ]
  ...
  const exportCsv = () => {
    ...
    link.setAttribute('download', `items-${from}-${to}.csv`)
  ...
  const filters = (
    <div className="grid grid-cols-1 md:grid-cols-5 gap-3">
      ...
      <div className="md:col-span-3">
        ...
        <input type="text" className="border rounded px-3 py-2 w-full" value={(params as any).q || ''} onChange={e => setParams({ q: e.target.value })} placeholder="Search item name or SKU…" />
      </div>
    </div>
  )
  ...
  return (
    <ReportLayout title="Items Report" description="Item sales quantity and amount within the selected period" breadcrumbs={breadcrumbs} actions={actions} filters={filters}>
      {/*
        ...
        // Inputs: from, to, optional search q in product name/SKU
        ...
        let items = rows.map(r => ({
          ...
          name: products.find(p => p.id === r.productId)?.name || 'Item',
          quantity: Number(r._sum.quantity || 0),
          amount: Number(r._sum.total || 0)
        }))
        if (query.q) items = items.filter(i => i.name?.toLowerCase().includes(query.q.toLowerCase()) || i.sku?.toLowerCase().includes(query.q.toLowerCase()))
        const totals = items.reduce((s, r) => ({ quantity: s.quantity + r.quantity, amount: s.amount + r.amount }), { quantity: 0, amount: 0 })
