...
export async function POST(request) {
  try {
    ...
    webhookLogger.info(`[Webhook] Received ${topic} event`)
    ...
    switch (topic) {
      ...
      default:
        webhookLogger.info(`[Webhook] Unhandled event type: ${topic}`)
    }
...
async function handleProductWebhook(productData) {
  webhookLogger.info(`[Webhook] Processing product: ${productData.id} - ${productData.name}`)
  ...
  webhookLogger.info(`[Webhook] Product ${productData.id} processed successfully`)
}

async function handleOrderWebhook(orderData) {
  webhookLogger.info(`[Webhook] Processing order: ${orderData.id}`)
