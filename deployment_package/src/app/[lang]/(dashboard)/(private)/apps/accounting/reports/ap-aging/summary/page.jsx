...
export default function APAgingSummaryPage() {
  ...
  const fetchData = async (date) => {
    ...
    try {
      ...
      const res = await fetch(`/api/accounting/reports/ap-aging/summary${qp.toString() ? `?${qp.toString()}` : ''}`)
      const json = await res.json()
      setData(json)
    } catch (e) {
      setError('Failed to load AP aging summary')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="max-w-5xl mx-auto bg-white border rounded shadow">
        ...
        <div className="text-center py-4">
          <div className="font-semibold">Account Payable Ageing Report (Summary)</div>
