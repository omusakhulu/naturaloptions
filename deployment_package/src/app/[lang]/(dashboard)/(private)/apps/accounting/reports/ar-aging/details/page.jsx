...
export default function ARAgingDetailsPage() {
  ...
  const fetchData = async (date) => {
    ...
    try {
      ...
      const res = await fetch(`/api/accounting/reports/ar-aging/details${qp.toString() ? `?${qp.toString()}` : ''}`)
      const json = await res.json()
      setData(json)
    } catch (e) {
      setError('Failed to load AR aging details')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className="p-6 space-y-4">
      ...
      <div className="max-w-5xl mx-auto bg-white border rounded shadow">
        ...
        <div className="text-center py-4">
          <div className="font-semibold">Account Receivable Ageing Details (Details)</div>
        ...
        <div className="px-4 pb-4">
          ...
          {data.items.map(row => (
            <div key={row.id} className="grid grid-cols-12 gap-2 px-2 py-2 border-b text-sm">
              <div className="col-span-3 truncate" title={row.customer}>{row.customer}</div>
