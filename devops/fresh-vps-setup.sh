...
sudo -u postgres psql -c \"SELECT count(*) as connections FROM pg_stat_activity WHERE datname='naturaloptions_db';\"
echo \"\"
echo \"Nginx Status:\"
...
remote_exec "systemctl status postgresql --no-pager | head -n 5"
...
echo -e "  • PostgreSQL installed and configured with connection pooling"
